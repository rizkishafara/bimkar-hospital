                ...
                <div class="max-w-xl">
                    <section>
                        ...
                        <form class="mt-6" id="formPeriksa"
                            action="{{ route('dokter.memeriksa.update', $janjiPeriksa->id) }}"
                            method="POST">
                            @csrf
                            @method('PATCH')
                            <div class="mb-3 form-group">
                                <label
                                    for="namaInput">Nama</label>
                            ...
                            <div class="mb-3 form-group">
                                ...
                                <input type="datetime-local"
                                    class="rounded form-control" id="tgl_periksa"
                                    name="tgl_periksa" value="{{ $janjiPeriksa->created_at }}" required>
                            </div>
                            <div class="mb-3 form-group">
                                <label
                                    for="catatan">Catatan</label>
                                <textarea class="rounded form-control" id="catatan" name="catatan" rows="3"
                                    placeholder="Catatan pemeriksaan">{{ $janjiPeriksa->periksa->catatan }}</textarea>
                            </div>
                            <div class="mb-3 form-group">
                                <label for="obat">Pilih
                                    Obat</label>
                                <select class="rounded form-control" id="obat" name="obat[]"
                                    onchange="hitungBiaya()" multiple>
                                    @foreach ($obats as $obat)
                                    <option value="{{ $obat->id }}" data-harga="{{ $obat->harga }}"
                                        @if(collect($janjiPeriksa->periksa->detailPeriksa)->pluck('id_obat')->contains($obat->id)) selected @endif>
